...
router.post('/task/:taskId', upload.single('file'), (req, res) => {
  ...
  const result = db.prepare('INSERT INTO attachments (task_id, user_id, filename, original_name, size) VALUES (?, ?, ?, ?, ?)').run(req.params.taskId, req.user.id, req.file.filename, req.file.originalname, req.file.size);
